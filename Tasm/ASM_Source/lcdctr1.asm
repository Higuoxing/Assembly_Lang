...
data segment
 ...
 io0832a        	DW 290h
 chlcdctlport		DW 2b0H	;字符lcd指令端口地址
 chlcddataport		DW 2b1H	;字符lcd数据端口地址
 ...
 mes   db 0ah,0dh,'PRESS UP or DOWN KEY IN THE PC KEYBOARD! ',0ah,0dh
       db 'LCD''s CONTRAST WILL BE CHANGE! END WITH OTHER KEY! ',0ah,0dh,'$'
 carriage_retrun db 0dh,'$'
 ; 图形点阵
...
.386
        ...
        mov ax,stacks
        mov ss,ax
	call char_lcd_reset	;字符液晶复位及初始化
	call graph_lcd_reset	;图形液晶复位及初始化
	call char_lcd_disp_line12	;显示两行字符
	call graph_lcd_disp_picture	;显示图形
	call char_lcd_contrast	;对比度控制
        mov ax,4c00h
	int 21h			;退出
...
char_lcd_disp_line12 proc near
	call delay2
	mov bl,10000000B
	call chlcd_write_ctr	;设置DDRAM地址
	mov cx,16
	mov bl,20h
char_lcd_disp_line12_loop:
	call chlcd_write_data
	inc bl
	loop char_lcd_disp_line12_loop
	...
	mov cx,16
	pop bx
char_lcd_disp_line12_loop1:
	call chlcd_write_data
	inc bl
	loop char_lcd_disp_line12_loop1
	ret
char_lcd_disp_line12 endp
...
char_lcd_contrast proc near	;对比度控制
	pusha
	pushf
...
char_lcd_contrast_loop:
	...
	mov al,cl
	call dispword
	mov dx,offset carriage_retrun
	mov ah,09
	int 21h
	mov ah,01h	;判断是否有键按下
	int 16h
	jz char_lcd_contrast_loop
	mov ah,0h
	int 16h
	cmp ah,50h
	jnz char_lcd_contrast_loop1
	dec cl
	jmp char_lcd_contrast_loop
char_lcd_contrast_loop1:
	cmp ah,48h
	jnz char_lcd_contrast_loop2
	inc cl
	jmp char_lcd_contrast_loop
char_lcd_contrast_loop2:
	popf
	popa
	ret
char_lcd_contrast endp
