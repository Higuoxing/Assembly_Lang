...
data segment
 ...
 io8253port0		DW 280h	;8253计数器0端口地址
 io8253port1		DW 281h	;8253计数器1端口地址
 io8253ctr		DW 283h	;8253计数器控制端口地址
 ...
 mes2  db 0ah,0dh,'PRESS UP or DOWN KEY IN THE PC KEYBOARD! ',0ah,0dh
       db 'LCD''s BACKLIGHT WILL BE CHANGE! END WITH ANY KEY! ',0ah,0dh,'$'
 ; 图形点阵
...
graph_lcd_backlight proc near	;背光控制
	mov dx,offset mes2	;显示提示信息
	mov ah,09
	int 21h
	mov  dx,word ptr ioport
	add  dx,io8253ctr	;向8253写控制字
	mov al,00010110B	;使0通道为工作方式3，写低8位字节
	out dx,al
	mov  dx,word ptr ioport
	add  dx,io8253port0
	mov al,255		;写入循环计数初值256
	out dx,al		;写入低字节
	mov  dx,word ptr ioport
	add  dx,io8253ctr
	mov al,01010010B	;设8253通道1工作方式1，写低8位字节
	out dx,al
	mov  dx,word ptr ioport
	add  dx,io8253port1
	mov cl,1		;写入循环计数初值1
	mov al,cl
	out dx,al		;写低字节
graph_lcd_backlight_loop:
	mov ah,01h	;判断是否有键按下
	int 16h
	jz graph_lcd_backlight_loop
	mov ah,0h
	int 16h
	cmp ah,48h
	jnz graph_lcd_backlight_loop1
	sub cl,16
	jmp graph_lcd_backlight_loop2
graph_lcd_backlight_loop1:
	cmp ah,50h
	jnz graph_lcd_backlight_exit
	add cl,16
graph_lcd_backlight_loop2:
	mov al,cl
	out dx,al		;写低字节
	jmp graph_lcd_backlight_loop
graph_lcd_backlight_exit:
	ret
graph_lcd_backlight endp
